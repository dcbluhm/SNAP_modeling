...
national_avg <- snap_2model |>
  filter(year <= 2019) |>  # <-- Filter data up to 2019
  group_by(year) |>
...
### Find highest and lowest participation states per year

```{r}
# Get highest and lowest states
...
# Get highest and lowest counties
county_extremes <- snap_2model |>
  group_by(year, NAME) |>
...
zero_participation_list <- snap_2model |>
  filter(snap_participation_rate == 0) |>
...
over_50_summary <- snap_2model |>
  filter(!is.na(snap_participation_rate)) |>
  group_by(year) |>
  summarize(
    total_counties = n(),
...
```{r}
# Step 1: Calculate national average per year
national_avg <- snap_2model |>
  group_by(year) |>
  summarize(national_avg_rate = mean(snap_participation_rate, na.rm = TRUE), .groups = "drop")

# Step 2: Join the national average back to the dataset
snap_with_avg <- snap_2model |>
  left_join(national_avg, by = "year")

# Step 3: Count total counties and those above the average
