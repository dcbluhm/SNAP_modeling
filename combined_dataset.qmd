...
rm(combined_poldb, county_observations, demographic_combined, fns_snap_county, fns_state_totals,
   ky_welfare, poldb, poldb_2023, ruralurbancodes2013, ruralurbancodes2023, snap_2015,
   snap_2016, snap_2017, snap_2018, snap_2019, snap_2023, snap_participation, state_fips, us_counties)
...
# set up folds
snap_folds <- rolling_origin(snap_train, initial = 3119, assess = 3119, cumulative = FALSE)
...
snap_rec <- 
  ...
  step_interact(terms = ~ poverty_pct:bbce_inclmt + median_household_income:bbce_inclmt) |> 
  step_normalize(all_numeric_predictors(), -c(snap_rate_acs, rural_urban_score, governor_is_democrat_1_yes, bbce)) |>
  step_dummy(all_nominal_predictors()) |>
  step_interact(terms = ~ unemployment_rate:starts_with("abawdwaiver"))

```

Linear Regression Electric Net
