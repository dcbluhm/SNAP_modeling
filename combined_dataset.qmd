...
snap_lm_rec <- 
  recipe(formula = snap_participation_rate ~ ., data = snap_train) |>
    ...
    step_mutate(bbce_inclmt = if_else(bbce_inclmt == -9, 130, bbce_inclmt),
              ...
              governor_is_democrat_1_yes = as.numeric(governor_is_democrat_1_yes),
              bbce_asset = factor(bbce_asset),
              abawdwaiver = factor(abawdwaiver)) |>
  step_dummy(all_nominal_predictors()) |>
  step_interact(terms = ~ unemployment_rate:starts_with("abawdwaiver") + poverty_pct:bbce_inclmt + median_household_income:bbce_inclmt)
...
snap_knn_rec <- 
  recipe(formula = snap_participation_rate ~ ., data = snap_train) |>
    ...
    step_mutate(bbce_inclmt = if_else(bbce_inclmt == -9, 130, bbce_inclmt),
              ...
              bbce_asset = factor(bbce_asset),
              abawdwaiver = factor(abawdwaiver)) |>
  step_normalize(all_numeric_predictors(), -c(snap_rate_acs, governor_is_democrat_1_yes)) |>
  step_dummy(all_nominal_predictors())
...
knn_wf <- workflow() |>
  ...
  add_recipe(recipe = snap_knn_rec)
